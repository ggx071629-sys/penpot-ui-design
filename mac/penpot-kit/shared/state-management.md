...
visual self-correction iterations per `shared/visual-self-review.md`). For multi-intent tasks also
record `objectives: [{ intent, target, dependencies, status }]`; the parent task resumes unfinished
authorized objectives after each skill returns. Record approvals in `decisions` with their concrete
proposal, target, scope, and user response so resume neither repeats an unchanged approval nor
infers approval for a changed proposal. A phase counter alone is not evidence of task completion.
