...
- Phase 1 checkpoint passed: `review` and `ambiguous` rows are approved (or dropped). `safe` rows have the scoped change-type opt-in required by the skill and were shown before writing; autofix mode alone does not supply authorization.
...
Verification checkpoint: show the export + counts and name the next batch within the already authorized map. Do not ask for another sign-off on unchanged approved rows. A changed scope or changed review/ambiguous row needs approval before its write; praise for one batch does not authorize such changes.
