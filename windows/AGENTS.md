...
- 项目级 Skills 在 `.agents/skills/`，使用普通目录副本，不依赖符号链接或管理员权限。`penpot-kit/skills/` 是唯一维护源；不要直接编辑生成的技能副本。修改源文件后运行 `python tools/sync-skills.py`，再运行 `python tools/workspace.py verify` 检查逐文件一致性。
