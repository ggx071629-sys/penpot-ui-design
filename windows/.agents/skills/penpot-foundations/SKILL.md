...
**Phase 1 — Primitives.** Propose the concrete set changes and token list (name, type, value,
tier, and activation plan), including color ramps and spacing/radius/type scales.
✋ Approval checkpoint: approve that list **before** creating or activating sets or adding tokens.
Reuse a recorded approval only for the same unchanged list. Then use `scripts/createTokenSet.js`
to create the approved `primitives` set and activate it (`set.toggleActive()` — new sets are inactive).
Verification: inspect `tokenOverview()` and naming; report discrepancies instead of requesting
approval again for the identical list.

**Phase 2 — Semantic.** Propose aliases with names, types, reference values, tiers, destination
sets, and activation changes. ✋ Approval checkpoint: approve the mapping **before** writing it.
Then add the approved aliases into two sets (referenced `primitives` must be active):
...
parallel colour set. Verification: read back the approved mapping and resolve discrepancies; an unchanged mapping does not need a second approval.

**Phase 3 — Dark mode (optional).** Propose the complete dark token values and theme/activation changes. ✋ Approval checkpoint: approve that proposal before writing. Then create the approved parallel **`modes/dark`** colour set (same colour names
...
(see `references/04-themes-modes.md`). Verification: inspect dark `resolvedValue`s, restore the approved light configuration, and
tell the user how to switch.

**Phase 4 — Apply / Infer / Bind.** First derive a concrete binding proposal with target shapes, properties, and token names. ✋ Approval checkpoint: obtain approval before binding unless the applicable scoped exact-swap opt-in already covers those rows. Inferred new tokens must go through their pre-creation approval above; inference alone grants no write permission. Then either bind tokens to a selection (`scripts/applyTokensToShapes.js`),
...
semantic-role review before applying uncertain rows. Verification: read back approved bindings and coverage; changed proposals need approval, unchanged approved rows do not.
...
## 11. User Checkpoints
| Timing | Artifacts shown | Approval or verification |
|--------|-----------------|--------------------------|
| 0 Discovery | existing tokens summary and scope | Confirm new / extend / infer scope; reuse an explicit unchanged scope already supplied |
| Before 1 Primitives writes | concrete set and token proposal | Approve names, types, values, tiers, and activation changes |
| Before 2 Semantic writes | concrete semantic→primitive mapping and set changes | Approve the proposed mapping |
| Before 3 Themes writes | dark token values and theme/activation proposal | Approve the proposed values and mode behavior |
| Before 4 Apply/Infer writes | target shapes/properties→token mapping | Approve bindings unless the applicable scoped exact-swap opt-in already covers them |
| After each approved write phase | read-back values, structure, and binding coverage | Verify and report; reapprove only changed proposals |
