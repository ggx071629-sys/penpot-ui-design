...
3. **Judge** — every checklist item passes → proceed to the ✋ user checkpoint, presenting the
   SAME image (don't re-export). Any item fails → group the known, independently understood
   defects covered by the current authorization; fix them with one logical change per
   `execute_code` call, then re-export and re-look once for the batch. Corrections restore the
   agreed implementation; new decisions or protected operations still require their approval.
4. **Bound it** — at most **2 self-correction iterations** per checkpoint. Still failing after 2?
   Stop further correction of this unit and present the image WITH the remaining defects named.
   Keep its approval-dependent successors pending, but continue other independently authorized
   work when the workflow permits it. Mark the task incomplete while required defects remain;
   never silently accept them or extend the two-round budget without approval.
...
- **One known-defect batch per iteration; one logical change per call.** Use targeted fixes
  for the defects found in the current inspection, not a fresh redesign or an expanding polish
  search. The two-iteration ceiling limits correction-and-verification rounds, not the number
  of independently understood defects fixed within an authorized round.
