...
- **Scope checkpoint:** show the inventory + stats. If the user already identified the scope, this is informational and inference proceeds. Ask only if page / board / file scope remains unresolved; read-only inference does not authorize renaming.
...
- ✋ **Checkpoint:** present the full before/after table grouped by bucket; get approval for the *review* and *ambiguous* rows. (Safe rows require the scoped change-type opt-in described in §9; autofix mode alone is not approval. Show them before writing, without another pause when that opt-in already applies.)
...
- **Verification checkpoint:** `export_shape('page','png','shape')` (or the renamed board), read-back names, and renamed/skipped/error counts. Report results and continue the already authorized map. New scope or changed review/ambiguous rows need approval before writing; an unchanged approved batch does not need another sign-off.
...
See `shared/modes-and-policies.md`. This skill's `mode-default` is **autofix**, but it only establishes eligibility. An explicit request to rename auto-generated layers in a specified scope supplies this change-type opt-in; an audit-only request or loading this skill does not. Show the plan before every write and preserve concrete approval for review/ambiguous rows:
...
|-------------|-----------------|-------------|
| 0 — Discovery | Layer inventory + name-status stats; scope | Informational if scope is already explicit; otherwise ask which page / board / file is intended |
...
| 2 — Renaming | `export_shape`, read-back names, and renamed/skipped/error counts | Verification/report only for the already authorized map; get approval before changed review/ambiguous rows or new scope |
