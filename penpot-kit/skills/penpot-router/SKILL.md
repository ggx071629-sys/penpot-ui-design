...
If target ambiguity materially changes the requested outcome, scope, or approval requirements, ask one disambiguating question. Resolve internal routing choices from the brief and read-only evidence when those boundaries are unchanged; do not ask the user to choose skill names as an implementation detail.
...
request maps cleanly to one target, continue. If two plausible targets materially differ in
outcome, scope, or required approval, **✋ Checkpoint:** ask one disambiguating question in plain
language. When only the internal technique differs, choose from the brief and live evidence and
state the route without adding a user decision.
...
3. Note every remaining requested intent, its dependencies, status, and next target; do not treat the queue as optional follow-up.
4. Invoke / instruct the target skill. This routing step ends here; the parent task remains responsible for all requested outcomes and performs subsequent routing as needed. The router performs no mutations.
**Exit criterion:** one active target named, contract emitted, and every remaining requested objective queued with dependencies and completion status. After the target returns, the parent task processes the next authorized item; a pending required objective prevents whole-task completion.
**✋ Checkpoint (conditional):** ask "Routing to X — confirm or redirect?" **only** when (a) Phase 1
still has an unresolved material ambiguity, or (b) the target is an expensive mutating
...
2. **One active writing target at a time.** Split multi-intent requests without dropping them. The parent task owns the queue and resumes the next authorized target after the current target completes, subject to dependencies and each target's approval gates.
3. **Read-only only.** The router never calls a mutating `execute_code` and never calls `export_shape`.
4. **Resolve material ambiguity.** Ask when target choices materially change outcome, scope, or approval; choose internal technique from evidence without requiring the user to select a skill.
...
| Phase 1 (classification) — only for unresolved material ambiguity | Outcomes/scopes of the candidate routes in plain language | Which outcome matches the request? Internal technique alone does not require a question |
| Phase 2 (handoff) — only if material ambiguity remains or the target is an expensive mutating flow on existing content | Chosen target + the Token-Aware Brief Contract + queued intents | "Routing to **X** — confirm or redirect?" (clean matches: state the route and proceed) |
...
| "The request mentions both tokens and a screen — I'll route to both at once." | Concurrent mutating skills can conflict. Dropping the second intent also leaves the request incomplete. | Use the matching multi-step workflow or route serially. Keep remaining objectives and dependencies in `storage.router.queuedIntents`; the parent task resumes them under their own approval gates. |
| "The routes imply different scopes, but I'll pick one and proceed." | A wrong-scope start can cause unapproved geometry or variant changes. | Ask at Phase 1 about the material scope difference. Do not repeat the question after it is answered; internal technique choices are resolved from evidence. |
