> PROJECT-LOCAL INSTALLATION: upstream installation/update/uninstall commands below are historical reference and disabled here. Use `docs/maintenance.md`. Do not reinstall from upstream or write global behavior.
...
`penpot-kit` (the "seed home"), so `shared/`/`policies/` relative links stay intact and the clone
...
| **Claude Desktop** | macOS `~/Library/Application Support/Claude/claude_desktop_config.json` · Win `%APPDATA%\Claude\claude_desktop_config.json` · Linux `~/.config/Claude/claude_desktop_config.json` | JSON `mcpServers` (stdio proxy) | `penpot-kit/dist/penpot-kit.instructions.md` → paste into a Project | inside that file |
| **Cursor** | `~/.cursor/mcp.json` | JSON `mcpServers` (**native HTTP** `url`) | per-project `<project>/.cursor/rules/penpot-kit.mdc` (`alwaysApply`) | `@penpot-kit/prompts/<name>.md` |
...
| **generic** | `penpot-kit/mcp.generic.json` (import into your client) | JSON `mcpServers` | `penpot-kit/dist/penpot-kit.instructions.md` or `AGENTS.md` | inside that file |
...
- Override the seed location with `PENPOT_KIT_HOME` if `penpot-kit` doesn't suit.
