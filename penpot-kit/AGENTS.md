...
Distinguish internal verification from user approval using `policies/approval-checkpoints.md`.
Complete verification and continue within the authorized phase; stop at every applicable named
user-approval checkpoint. "Looks good" approves only the phase just shown, not future phases.
Record and reuse approval of an unchanged concrete proposal; do not request the same approval twice.
Always state the next phase explicitly. Existing per-frame, per-section, and high-risk gates remain.
