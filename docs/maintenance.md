...
| 现象 / 检查项 | 处理方法 |
|---|---|
| `python3` 命令不存在 | 安装 Python 3.9+ 并重新打开终端；Windows 若使用 `py -3`，用它替换文档中的 `python3`。Windows 尚未实测。 |
...
| `project_skill_links` 失败 | 检查是否漏了隐藏 `.agents/`，或解压把链接变成普通文本。单个链接如 `.agents/skills/penpot-router` 应指向 `../../penpot-kit/skills/penpot-router`。优先重新获取保留符号链接的完整副本，不覆盖已有未知文件。 |
